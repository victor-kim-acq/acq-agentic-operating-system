...
export async function GET(req: NextRequest) {
  ...
  try {
      ...
      const result = await sql`
        SELECT
          ...
          SUM(CASE WHEN EXISTS (
            SELECT 1 FROM deal_membership dm
            JOIN memberships m ON m.membership_id = dm.membership_id
            WHERE dm.deal_id = d.deal_id AND m.status = 'Active'
          ...
          SUM(CASE WHEN NOT EXISTS (
            ...
            SELECT 1 FROM deal_membership dm
            JOIN memberships m ON m.membership_id = dm.membership_id
            WHERE dm.deal_id = d.deal_id AND m.status = 'Cancellation'
    ...
    return NextResponse.json(
      { error: "Failed to fetch sold vs collected data" },
      { status: 500 }
    );
  }
}
